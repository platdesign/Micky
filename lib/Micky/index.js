	...
	function($http, Hawk, Oz, $localStorage, $q, $rootScope) {
		...
		function Client(options) {
			...
			this.authenticate = function(credentials) {
				...
				return request('POST', options.routes.signin, credentials)
				.then(function(res) {
			...
			autoressiuseAuthTicket();

			function autoressiuseAuthTicket() {
				reissueTicket('auth');
				setTimeout(autoressiuseAuthTicket, 1000 * 10);
			}
